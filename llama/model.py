...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None, video_start=None):
        ...
        if adapter is not None:            
            ...
            # Zitian: I don't know why but let's do the same thing
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args):
        ...
        if self.dlr:
            self.query_tokens = nn.ParameterDict({
                m: nn.Parameter(torch.randn(params.dim))
        ...
        if args.vocab_path is not None:
            self.use_vocab = True
            if args.vocab_path == 'all_words':
                self.register_buffer('projector_vocab', self.tok_embeddings.weight[:, None])
            else:
                self.register_buffer('projector_vocab', torch.load(args.vocab_path))
            print('projector_vocab shape:', self.projector_vocab.shape)
            self.projector_vocab = set_precision(self.projector_vocab, self.precision)
            self.n_projector_vocab = self.projector_vocab.shape[0]
            self.mm_projs_heads = nn.ModuleDict({
                m: nn.Sequential(
                    nn.GELU(),
                    nn.Linear(params.dim, self.n_projector_vocab)
                )
                for m in self.mm_used
            })
        else:
        ...
        if args.zero_init:
            for m in self.mm_used:
                if args.proj_type == 'linear':
                    ...
                    print('zero intialized:', self.mm_projs[m].weight.shape)
                else:
                    nn.init.constant_(self.mm_projs[m][-1].weight, 0.0)
                    print('init last linear layer')
                    print('zero intialized:', self.mm_projs[m][-1].weight.shape)
    ...
    def dlr_first_pass(self, data, inference=False):
        ...
        for i in range(bsz):
            for m, st in mm_starts[i]:
                # print('add q token', m, st)
                feats = self.query_tokens[m].unsqueeze(0) # 1 x n_dim
                # qt_type = feats.dtype
                feats = set_precision(feats, self.precision)
        ...
        vqa_h = self.norm(vqa_h) # btz x len x n_dim?

        vqa_h = vqa_h.reshape(bsz, n_options, seqlen, self.params.dim)
    
        # print('vqa_h shape', vqa_h.shape)

        mm_features = []
        for i in range(bsz):
            d = {m: [] for m in self.mm_used}
            for m, st in mm_starts[i]:
                d[m].append(vqa_h[i, 0, st: st+1].float())
            mm_features.append(d)
    ...
    def forward(self, data, use_vis, inference=False):
        ...
        if self.dlr:
            mm_features = self.dlr_first_pass(data, inference)
        else:
        ...
        for m in self.mm_used:
            ...
            for sample in mm_features:
                if len(sample[m]) > 0:
                    ...
                    # print('got one mm list of length 0')
                    feats.append(torch.Tensor([]).cuda())
            # feats = [torch.cat(sample[m], dim=0).cuda() for sample in mm_features]
            split_sizes_ = [x.shape[0] for x in feats]
            feats = torch.cat(feats, dim=0)
            # ('feats shape', m, feats.shape)
            if feats.shape[0] == 0:
                continue
            # feats = set_precision(feats, self.precision)
            if not self.use_vocab:
                ...
                # print(feats_proj.norm(dim=-1))
                # print('mean norm:', feats_proj.norm(dim=-1).mean())
                fnorm = feats_proj.norm(dim=-1).mean()
            else:
                x = self.mm_projs[m](feats) # n x dim
                logits = self.mm_projs_heads[m](x) # n x vocab
                logits = set_precision(logits, self.precision)
                selection_mask = F.gumbel_softmax(logits, dim=-1, hard=True) # n x vocab
                emb = selection_mask[..., None, None] * self.projector_vocab[None] # n x vocab x max_feats x dim
                # print('emb shape:', emb.shape)
                feats_proj = emb.sum(dim=1).reshape(-1, emb.shape[-1])
            ...
            # print('feats_proj shape', m, feats_proj.shape)
